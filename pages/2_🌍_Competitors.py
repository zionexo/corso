...
conf_turno=alt.Chart(conf).mark_line(point=True).encode(x=alt.X('turno',axis=alt.Axis(tickMinStep=1),title='TURNO'),color='Indicatori aziendali')
...
score=conf_turno.encode(y=alt.Y('pos',axis=alt.Axis(tickMinStep=1),scale=alt.Scale(reverse=True , domain=[1,5]),title='Posizione'))
